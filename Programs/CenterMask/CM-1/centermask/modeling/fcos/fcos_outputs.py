...
class FCOSOutputs(object):
    ...
    def predict_proposals(self):
        ...
        if debug_flag:
                no_of_proposals_per_level = []

        for i, (l, o, r, c, s) in enumerate(zip(*bundle)):
            # recall that during training, we normalize regression targets with FPN's stride.
            # we denormalize them here.
            r = r * s
            #MB - 20250816 - "sampled_boxes.append(" split; debug_flag and detailed_debug_flag contents added
            #sampled_boxes.append(
            #    self.forward_for_single_feature_map(
            #        l, o, r, c, self.image_sizes
            #    )
            #)
            proposals_per_fpn_level = self.forward_for_single_feature_map(
                l, o, r, c, self.image_sizes
            )
            sampled_boxes.append(proposals_per_fpn_level)
            if debug_flag:
                no_of_proposals_per_level.append(len(proposals_per_fpn_level[0].pred_boxes))

            if detailed_debug_flag:
                tmp = self.forward_for_single_feature_map(l, o, r, c, self.image_sizes)
                print(f"\t FPN level: {i}; # images/batch: {len(tmp)}")
                print(f"\t\t type(tmp): {type(tmp)}; type(tmp[0]): {type(tmp[0])}")
                #print(f"\t\t tmp[0]._fields: {tmp[0]._fields}")
                print(f"\t\t len(tmp[0].pred_boxes): {len(tmp[0].pred_boxes)}")

        if debug_flag:
            total_no_of_proposals = sum(no_of_proposals_per_level)
            if total_no_of_proposals > 0:
                normalized_counts_per_level = [x/total_no_of_proposals*100 for x in no_of_proposals_per_level]

                proposal_bundle = (no_of_proposals_per_level, normalized_counts_per_level)
                for i, (count, ncount) in enumerate(zip(*proposal_bundle)):
                    print(f"FPN level {i}: {count} ({ncount:.1f}%); \t")
            else:
