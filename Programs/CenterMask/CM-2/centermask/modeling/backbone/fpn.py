...
class LastLevelP5P6(nn.Module):
    """
    MB - 20240814 - Modified version of LastLevelP6P7 - This module is used in RetinaNet and FCOS to 
    generate extra layers, P5 and P6 from xx?
    #Description of p6p7 - This module is used in RetinaNet and FCOS to generate extra layers, P6 and P7 from
    #C5 or P5 feature.
    """
    ...
    def __init__(self, in_channels, out_channels, in_features="res5"):
        ...
        self.num_levels = 2
        self.in_feature = in_features
        self.p5 = nn.Conv2d(in_channels, out_channels, 3, 2, 1)
        self.p6 = nn.Conv2d(out_channels, out_channels, 3, 2, 1)
        for module in [self.p5, self.p6]:
            weight_init.c2_xavier_fill(module)
...
def build_fcos_resnet_fpn_backbone(cfg, input_shape: ShapeSpec):
    ...
    if top_levels == 2:
        if cfg.MODEL.FCOS.IN_FEATURES[3] == "p5":
            top_block = LastLevelP5P6(in_channels_top, out_channels, "p4")
        elif cfg.MODEL.FCOS.IN_FEATURES[3] == "p6":
            top_block = LastLevelP6P7(in_channels_top, out_channels, "p5")
        else:
            raise ValueError("At present top levels must be p5 or p6")
    if top_levels == 1:
