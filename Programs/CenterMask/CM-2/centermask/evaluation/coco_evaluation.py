...
class COCOEvaluator(DatasetEvaluator):
    ...
    def _derive_coco_results(self, coco_eval, iou_type, class_names=None):
        ...
        #print(f"class_names:{class_names}; precisions.shape[2]:{precisions.shape[2]}")
        assert len(class_names) == precisions.shape[2]

        #MB: computer per-category average recall
        recalls = coco_eval.eval["recall"] #dimension iou_thrs x catIds x area x maxDets
        ...
        for idx, name in enumerate(class_names):
            ...
            print(f"results_per_category: {results_per_category}")
            print(f"{name}: precision at IoU = 0.5 and all recall levels = {precisions[0, :, idx, 0, -1]}")
            ...
            print(f"recall_results_per_category: {recall_results_per_category}")
            print(f"{name}: recall at IoU = 0.5: {recalls[0, idx, 0, -1]}")
...
def _evaluate_predictions_on_coco(
    ...
    coco_eval.params.maxDets = [100, 300, 600]
