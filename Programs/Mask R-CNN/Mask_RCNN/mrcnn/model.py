...
def load_image_gt(dataset, config, image_id, augment=False, augmentation=None,
    ...
    """
    # Load image and mask
    #logging.info("image_info:{1}".format(1=dataset.image_info[image_id]['path'])
    ...
    #MB Jan 08, 2023: To account for COCO annotator annotation error
    ...
    #ind_cat1 = (class_ids == 1)
    #ind_cat2 = (class_ids == 2)
    #class_ids[ind_cat1] = 2
    #class_ids[ind_cat2] = 1
    #mb
...
class MaskRCNN():
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        # Train
        log("\nStarting at epoch {}. LR={}. LM={}.\n".format(self.epoch, learning_rate, self.config.LEARNING_MOMENTUM))
...
#MB Jan 04, 2023
#Custom Callback
#Ref: https://github.com/matterport/Mask_RCNN/issues/1839
class MeanF1Callback(Callback):
    
    def __init__(self, train_model: MaskRCNN, inference_model: MaskRCNN, dataset: utils.Dataset,
                 calculate_metrics_at_every_X_epochs=3, dataset_limit=None, iou_threshold=0.5,
                 verbose=1):
        super().__init__()
        self.train_model = train_model
        self.inference_model = inference_model
        self.dataset = dataset
        self.calculate_metrics_at_every_X_epochs = calculate_metrics_at_every_X_epochs
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if epoch > 2 and (epoch+1)%self.calculate_metrics_at_every_X_epochs == 0:
            self._verbose_print("Calculating mean F1 ...")
            self._load_weights_for_model()

            c1_f1_score, c2_f1_score = self._evaluate_multiclass_f1_metrics()

            if logs is not None:
                logs["c1_f1_score"] = c1_f1_score
                logs["c2_f1_score"] = c2_f1_score

            self._verbose_print("F1-score at epoch {0} is: C1={1}; C2={2}".format(epoch+1, c1_f1_score, c2_f1_score))
    ...
    def _multiclass_eval_metrics(self, class_id, gt_class_ids, pred_class_ids, gt_match):
    
        #Note: np.count_nonzero() faster than np.sum()
        
        gt_positives = np.count_nonzero(gt_class_ids == class_id) #TP + FN
        ind = (gt_class_ids == class_id) #gt and pred match for the given class
        pred_match = gt_match[ind]
        pred_true_positives = np.count_nonzero(pred_match != -1) #TP
        pred_total_positives = np.count_nonzero(pred_class_ids == class_id) #TP + FP

        precision = pred_true_positives / pred_total_positives if pred_total_positives != 0 else 0
        recall = pred_true_positives / gt_positives if gt_positives != 0 else 0
    ...
    def _evaluate_multiclass_metrics(self):
        ...
        for image_id in self.dataset_image_ids[:self.dataset_limit]:
            original_image, image_meta, gt_class_id, gt_bbox, gt_mask =\
                load_image_gt(self.dataset, self.inference_model.inference_config, 
            ...
            result = self.inference_model.detect([image_array])[0]
            #
            ...
            class_id = 1
            c1_precision, c1_recall, c1_f1_score = self._multiclass_eval_metrics(class_id, gt_class_id, pred_class_ids, 
                                                                  gt_match)
            #print(f"image_id: {image_id}; IoU threshold: {iou_thresh}")
            #print(f"\t class: {class_id}; precision: {c1_precision}; recall: {c1_recall}; c1_f1_score: {c1_f1_score}")
            class_id = 2
            c2_precision, c2_recall, c2_f1_score = self._multiclass_eval_metrics(class_id, gt_class_id, pred_class_ids, 
                                                                  gt_match)
            #print(f"\t class: {class_id}; precision: {c2_precision}; recall: {c2_recall}; c1_f1_score: {c2_f1_score}")
        ...
        return c1_mean_precision, c1_mean_recall, c1_f1_score, c2_mean_precision, c2_mean_recall, c2_f1_score

    def _evaluate_multiclass_f1_metrics(self):
        ...
        for image_id in self.dataset_image_ids[:self.dataset_limit]:
            original_image, image_meta, gt_class_id, gt_bbox, gt_mask =\
                load_image_gt(self.dataset, self.inference_model.config, 
                                       image_id, use_mini_mask=False)
            ...
            class_id = 1
            _, _, c1_f1_score = self._multiclass_eval_metrics(class_id, gt_class_id, pred_class_ids, 
                                                                  gt_match)
            #print(f"image_id: {image_id}; IoU threshold: {iou_thresh}")
            #print(f"\t class: {class_id}; precision: {c1_precision}; recall: {c1_recall}; c1_f1_score: {c1_f1_score}")
            class_id = 2
            _, _, c2_f1_score = self._multiclass_eval_metrics(class_id, gt_class_id, pred_class_ids, 
                                                                  gt_match)
            #print(f"\t class: {class_id}; precision: {c2_precision}; recall: {c2_recall}; c1_f1_score: {c2_f1_score}")
        ...
        return c1_f1_score, c2_f1_score
#mb